...
public class DrainableContainer : MonoBehaviour
{
	...
	public class PunctureHole
	{
		...
		private float baseFlowRate;
		public float TimeStopped { get; private set; }
		
		public PunctureHole(Vector3 position, Vector3 direction, float radius, float bfr, DrainableContainer container)
		{
			baseFlowRate = bfr;
			Container = container;
			Position = position;
			...
			Particles.Play();
			TimeStopped = -1;
			Particles.startSize = Radius * 8; //magic, controls particle size based on hole size
		}
		public float Leak()
		{
			...
			Particles.emissionRate = Radius * baseFlowRate * velocity;
	...
	public void AddHole(Vector3 position, Vector3 normal, float radius = -1)
	{
		...
		{
			var holeToAdd = new PunctureHole(position, -normal, radius, BaseFlowRate, this);
			holes.Add(holeToAdd);
		}
	}
